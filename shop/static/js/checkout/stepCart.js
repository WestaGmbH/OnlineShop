        ...
        function updateCarouselItems(){
           ...
           if (filteredItems.length > 0){
               ...
               filteredItems.forEach((item, index) => {
                  ...
                  carouselContainer.appendChild(createProductCard(false, item, index, allItems, filteredItems, favouriteItems, window.config.preOrderIconUrl, window.config.silverIconUrl, window.config.steelIconUrl, vocabulary, translations_categories, currency, window.config.changeFavouritesStateUrl, show_quantities, window.config.addToCatalogUrl, getCookie('csrftoken'), window.config.cartUrl, window.config.isAuthenticated, window.config.shopPageUrl, true));
               });
